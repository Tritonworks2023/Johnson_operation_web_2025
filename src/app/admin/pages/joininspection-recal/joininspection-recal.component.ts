...
@Component({
  selector: 'app-joininspection-recal',
  templateUrl: './joininspection-recal.component.html',
  styleUrls: ['./joininspection-recal.component.css']
})
export class JoininspectionRecalComponent implements OnInit {
  ...
  listpettype() {
    this._api.Joint_inspection_jobdetail_list_two().subscribe(
      (response: any) => {
  ...
  Editcompanydetailsdata(data) {
    ...
    update_join_key(data){
      let a  = {
        SMU_JOBNO : data.SMU_JOBNO,
        SMU_TECHMOBNO: data.SMU_TECHMOBNO,
        SMU_UKEY : data.SMU_UKEY,
      }
      this._api.update_join_key_data(a).subscribe(
        (response: any) => {
         this.ngOnInit();
        }
      );

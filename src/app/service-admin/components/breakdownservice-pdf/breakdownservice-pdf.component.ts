...
export class BreakdownservicePdfComponent implements OnInit {
  ...
  bottom_text = 'IF DOOR OPEN KEY IS USED,ENSURE THAT DOOR IS FULLY CLOSED.';
  ...
  ngOnInit(): void {
    ...
    if(job_detail.job_id[0] == 'L'){
      this.bottom_text = 'CAUTION : IF DOOR OPEN KEY IS USED,ENSURE THAT DOOR IS FULLY CLOSED.';
    }else{
      this.bottom_text = '';
    }
    let datas = {
    ...
    this._api.breakdown_data_details(datas).subscribe((response: any) => {
      ...
      let d = {
        feedback_details : this.storaged_data.feedback_details
      }
      this._api.fetch_area_breakdown(d).subscribe((response: any) => {
      });
